...
def backyard_main(organization_id, workspace_id):
    ...
    result, msg = set_module_vars_link(objdbca, TFCloudEPConst)  # noqa: F405
    if not result:
        # ####メモ：Module-変数紐付テーブルの更新に失敗。ログに出力
        g.applogger.error("Module-変数紐付管理テーブルの更新に失敗")
        g.applogger.error(msg)
        return

    # Movement-Module紐付に登録されているレコードから、Movement-Module変数紐付テーブルを更新する
    result, msg = set_movement_var_link(objdbca, TFCloudEPConst)  # noqa: F405
    if not result:
        # ####メモ：Module-変数紐付テーブルの更新に失敗。ログに出力
    ...
    result, msg = set_movement_var_member_link(objdbca, TFCloudEPConst)  # noqa: F405
    if not result:
        # ####メモ：Module-変数紐付テーブルの更新に失敗。ログに出力
        g.applogger.error("Movement-メンバー変数紐付テーブルの更新に失敗")
        g.applogger.error(msg)
        return
