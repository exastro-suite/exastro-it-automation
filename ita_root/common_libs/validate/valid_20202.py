...
def external_valid_menu_after(objDBCA, objtable, option):
    ...
    # 登録/更新時の場合、Playbookの正常性チェック
        ...
        playbook_data_decoded = playbook_data_binary.decode('utf-8')
        filepath_tmp = "%s/20202_playbook_file_%s.yml" % (get_AnsibleDriverTmpPath(), os.getpid())
        ...
        # BOM有無をチェック
        elif playbook_data_binary[0:3] == b'\xef\xbb\xbf':
            retBool = False
