...
class AuthTypeParameterRequiredCheck:
    ...
    def __init__(self):
        ...
        # ERROR_TYPE12: 証明書認証(winrm)の場合、winrm公開鍵ファイルは必須項目です。",
        # ERROR_TYPE13: 証明書認証(winrm)の場合、winrm秘密鍵ファイルは必須項目です。",
        ...
        self.err_msg_code_ary[AuthTypeParameterRequiredCheck.chkType_Loadtable_DeviceList]['ERROR_TYPE12'] = "MSG-10941"
        self.err_msg_code_ary[AuthTypeParameterRequiredCheck.chkType_Loadtable_DeviceList]['ERROR_TYPE13'] = "MSG-10942"
...
#        self.err_msg_code_ary[AuthTypeParameterRequiredCheck.chkType_WorkflowExec_TowerHostList]['ERROR_TYPE11'] = "MSG-10231"
    ...
    def DeviceListAuthTypeRequiredParameterCheck(self, chk_type, err_msg_parameter_ary, str_auth_mode, str_login_user, str_passwd, str_ssh_key_file, str_passphrase, driver_id, str_winrm_private_key_file, str_winrm_public_key_file):
        """
        処理内容
        ...
        ARGS:
            ...
            str_protocol_id:        Pioneerプロトコル
                                        ...
                                        1:  telnet
                                        2:  ssh
            str_winrm_private_key_file:  winrm秘密鍵ファイル
            str_winrm_public_key_file:  winrm公開鍵ファイル
        ...
        if str_auth_mode == AnscConst.DF_LOGIN_AUTH_TYPE_KEY:  # 認証方式:鍵認証(パスフレーズなし)
            if not str_ssh_key_file:
                # "認証方式が鍵認証(パスフレーズなし)の場合、秘密鍵ファイル(id_rsa)は必須項目です。",
            ...
            if not str_passwd:
                # "認証方式がパスワード認証の場合、ログインパスワードは必須項目です。",
            ...
            if not str_ssh_key_file:
                ...
                # "認証方式が鍵認証(パスフレーズあり)の場合、秘密鍵ファイル(id_rsa)は必須項目です。",
                result = g.appmsg.get_api_message(error_cde, [err_msg_parameter_ary])
            if not str_passphrase:
                # "認証方式が鍵認証(パスフレーズあり)の場合、パスフレーズは必須項目です。",
                error_cde = self.err_msg_code_ary[chk_type]['ERROR_TYPE4']
                if len(result) != 0:
                    result += "\n"
                result += g.appmsg.get_api_message(error_cde, [err_msg_parameter_ary])
        elif str_auth_mode == AnscConst.DF_LOGIN_AUTH_TYPE_KEY_WINRM:  #  証明書認証(winrm)
            if not str_winrm_private_key_file:
                # "証明書認証(winrm)の場合、winrm公開鍵ファイルは必須項目です。",
                error_cde = self.err_msg_code_ary[chk_type]['ERROR_TYPE13']
                if len(result) != 0:
                    result += "\n"
                result += g.appmsg.get_api_message(error_cde, [err_msg_parameter_ary])
                # "証明書認証(winrm)の場合、winrm秘密鍵ファイルは必須項目です。",
            if not str_winrm_public_key_file:
                # "証明書認証(winrm)の場合、winrm秘密鍵ファイルは必須項目です。",
                error_cde = self.err_msg_code_ary[chk_type]['ERROR_TYPE12']
                if len(result) != 0:
                    result += "\n"
                result += g.appmsg.get_api_message(error_cde, [err_msg_parameter_ary])
