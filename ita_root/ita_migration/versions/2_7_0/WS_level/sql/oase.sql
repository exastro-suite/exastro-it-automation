...
-- 重複排除機能の追加によってエージェント毎に収集経過を管理する必要があるため、収集経過管理テーブルにエージェントIDを追加
ALTER TABLE T_OASE_EVENT_COLLECTION_PROGRESS ADD COLUMN AGENT_NAME VARCHAR(1024) AFTER EVENT_COLLECTION_SETTINGS_ID;
...
-- 個別アラートキューイング対応
-- イベント種別マスタのEVENT_TYPE_IDのサイズ変更(2 → 255)
ALTER TABLE T_OASE_EVENT_TYPE MODIFY COLUMN EVENT_TYPE_ID VARCHAR(255);
