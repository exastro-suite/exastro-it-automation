...
def external_valid_menu_before(objdbca, objtable, option):  # noqa: C901
    ...
    ret = objdbca.table_select(TFCLIEPConst.V_COLUMN_LIST, where_str, [menu_group_menu_item, 0])
    if not ret:
    ...
    if registration_method == TFCLIEPConst.COL_TYPE_KEY and not str(hcl_setting) == "0":
        retBool = False
    ...
    ret = objdbca.table_select(TFCLIEPConst.T_MOVEMENT_VAR, where_str, [variable_name, 0])
    if not ret:
        # IDColumnに選択不可能な値であるため、loadTable側でエラー判定させる。
        return retBool, msg, option
    ...
    if member_variable_name:
        ...
        ret = objdbca.table_select(TFCLIEPConst.T_MOVEMENT_VAR_MEMBER, where_str, [member_variable_name, 0])
        if not ret:
            # IDColumnに選択不可能な値であるため、loadTable側でエラー判定させる。
            return retBool, msg, option
    ...
    ret = objdbca.table_select(TFCLIEPConst.T_MODULE_VAR, where_str, [mvmt_var_module_vars_link_id, 0])
    if not ret:
        # IDColumnに選択不可能な値であるため、loadTable側でエラー判定させる。
        return retBool, msg, option
    ...
    ret = objdbca.table_select(TFCLIEPConst.T_TYPE_MASTER, where_str, [var_type_id, 0])
    if not ret:
        retBool = False
    ...
    if cmd_type == 'Update':
        ...
        ret = objdbca.table_select(TFCLIEPConst.T_VALUE_AUTOREG, where_str, [variable_name, 0, item_no])
    else:
        # 「登録」「復活」の場合の検索条件
        where_str = 'WHERE MVMT_VAR_LINK_ID = %s AND DISUSE_FLAG = %s'
        ret = objdbca.table_select(TFCLIEPConst.T_VALUE_AUTOREG, where_str, [variable_name, 0])
