...
def get_upload_file_path_specify(workspace_id, place, uuid, file_name, uuid_jnl):
    ...
    """
    organization_id = g.get("ORGANIZATION_ID")
    file_path = os.path.join(os.environ.get("STORAGEPATH"), organization_id, f"{workspace_id}{place}", uuid, file_name)
    old_file_path = ""
    if uuid_jnl is not None:
        if len(uuid_jnl) > 0:
            old_file_path = os.path.join(os.environ.get("STORAGEPATH"), organization_id, f"{workspace_id}{place}", uuid, "old", uuid_jnl, file_name)  # noqa: E501
