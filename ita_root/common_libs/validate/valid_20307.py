            ...
            if len(row) == 1:
                if row[0]['MENU_CNT'] == 1:
                    if row[0]['COLUMN_LIST_ID_CNT'] == 1:
                        ...
                        table_name = "T_ANSP_VALUE_AUTOREG"
        ...
        if chk_pattern_id and rg_vars_link_id:
            ...
            if len(row) == 1:
                if row[0]['VARS_LINK_ID_CNT'] == 1:
                    ...
                    boolExecuteContinue = False
                    # Movement-対話種別紐付に登録されている対話種別に変数が未登録です。
                    msg = g.appmsg.get_api_message("MSG-10393")
                    retBool = False
                    boolExecuteContinue = False
            else:
                boolSystemErrorFlag = True
