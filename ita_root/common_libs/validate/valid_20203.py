...
def external_valid_menu_after(objdbca, objtable, option):
    ...
    if option["cmd_type"] == "Register" or option["cmd_type"] == "Update" or option["cmd_type"] == "Restore":
        table_name = "T_ANSL_MVMT_MATL_LINK"
        where_str = "WHERE LINK_ID <> %s AND MOVEMENT_ID = %s AND PLAYBOOK_MATTER_ID <> %s  AND DISUSE_FLAG = 0 "
        aryForBind = {}
        aryForBind['LINK_ID'] = option["uuid"]
        if option["cmd_type"] == "Restore":
            aryForBind['MOVEMENT_ID'] = option["current_parameter"]["parameter"]["movement"]
            aryForBind['PLAYBOOK_MATTER_ID'] = option["current_parameter"]["parameter"]["role_package_name"]
        else:
            aryForBind['MOVEMENT_ID'] = option["entry_parameter"]["parameter"]["movement"]
            aryForBind['PLAYBOOK_MATTER_ID'] = option["entry_parameter"]["parameter"]["role_package_name"]
