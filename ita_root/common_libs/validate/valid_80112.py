...
def external_valid_menu_before(objdbca, objtable, option):  # noqa: C901
    ...
    # 「メニューグループ:メニュー:項目(menu_group_menu_item)」で選択したメニューがバンドル利用かどうかを判定
    ...
    # バンドル利用である場合「代入順序(column_substitution_order)」の値があること、バンドル利用でない場合「代入順序(column_substitution_order)」の値がないことを判定
    if str(vertical_flag) == "0":
        # バンドル利用ではない(0)場合に、「代入順序(column_substitution_order)」に値があればバリデーションエラー
        if column_substitution_order:
            retBool = False
            msg = g.appmsg.get_api_message("MSG-80008")
            return retBool, msg, option,
    else:
        # バンドル利用(1)の場合に、「代入順序(column_substitution_order)」に値が無ければバリデーションエラー
    ...
    if registration_method == TFCloudEPConst.COL_TYPE_KEY and not str(hcl_setting) == "0":
        retBool = False
    ...
    ret = objdbca.table_select(TFCloudEPConst.T_MOVEMENT_VAR, where_str, [variable_name, 0])
    if not ret:
        # IDColumnに選択不可能な値であるため、loadTable側でエラー判定させる。
        return retBool, msg, option
    ...
    if member_variable_name:
        ...
        ret = objdbca.table_select(TFCloudEPConst.T_MOVEMENT_VAR_MEMBER, where_str, [member_variable_name, 0])
        if not ret:
            # IDColumnに選択不可能な値であるため、loadTable側でエラー判定させる。
            return retBool, msg, option
    ...
    ret = objdbca.table_select(TFCloudEPConst.T_MODULE_VAR, where_str, [mvmt_var_module_vars_link_id, 0])
    if not ret:
        # IDColumnに選択不可能な値であるため、loadTable側でエラー判定させる。
        return retBool, msg, option
    ...
    if cmd_type == 'Update':
        ...
        ret = objdbca.table_select(TFCloudEPConst.T_VALUE_AUTOREG, where_str, [variable_name, 0, item_no])
    else:
        # 「登録」「復活」の場合の検索条件
        where_str = 'WHERE MVMT_VAR_LINK_ID = %s AND DISUSE_FLAG = %s'
        ret = objdbca.table_select(TFCloudEPConst.T_VALUE_AUTOREG, where_str, [variable_name, 0])
